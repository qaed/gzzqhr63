...
import nc.ui.hr.formula.variable.PsndocHrVariableCreator;
import nc.ui.hr.formula.variable.PsnjobHrVariableCreator;
...
public class WabaItemFormularFactory extends HrWaDefaultVariableFactory {
	...
	protected List<FormulaItem> createAllFieldItems(String tablename) {
		...
		if (WA_ITEM_DES.equals(tablename)) {
			fieldItems = new WaItemVaribleItemCreator().createFormulaItems(getcontex());

		} else if ("奖金分配项目".equals(tablename)) {
			fieldItems = new WaBaItemVaribleItemCreator().createFormulaItems(getcontex());
		}
