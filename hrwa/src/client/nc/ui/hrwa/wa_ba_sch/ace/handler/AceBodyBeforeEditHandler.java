...
public class AceBodyBeforeEditHandler implements IAppEventHandler<CardBodyBeforeEditEvent> {
	...
	public void handleAppEvent(CardBodyBeforeEditEvent e) {
		if (e.getKey().equals("ba_unit_code")) {
			...
			uiref.getRefModel().setWherePart("isnull(dr,0)=0 and pk_org='" + e.getContext().getPk_org() + "' and pk_wa_ba_unit not in (select distinct unit.ba_unit_code from wa_ba_sch_unit unit left join wa_ba_sch_h h on unit.pk_ba_sch_h =h.pk_ba_sch_h where h.cperiod = '" + cperiod + "' and h.cyear = '" + cyear + "' and isnull(unit.dr,0)=0 )");
