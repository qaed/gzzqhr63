...
public class HttpHelper {
	...
	public static JSONObject httpGet(String url) throws OApiException {
		...
		*/
		...
		try {
			...
			throw new OApiException(-1, e.getMessage());
		} finally {
			if (response != null)
				try {
					response.close();
	...
	public static JSONObject httpPost(String url, Object data) throws OApiException {
		...
		try {
			...
			System.out.println("request url=" + url + ", exception, msg=" + e.getMessage());
			throw new OApiException(-2, "请求失败,"+e.getMessage());
		} finally {
			if (response != null)
				try {
					response.close();
	...
	public static JSONObject uploadMedia(String url, File file) throws OApiException {
		...
		HttpEntity requestEntity =
				MultipartEntityBuilder.create().addPart("media", new FileBody(file, ContentType.APPLICATION_OCTET_STREAM, file.getName())).build();
		httpPost.setEntity(requestEntity);
