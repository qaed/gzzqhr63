...
public class SyncDept implements IBackgroundWorkPlugin {
	...
	private PreAlertObject syncTask(BgWorkingContext arg0) throws BusinessException {
		...
		// 领袖资本
		//		sql.delete(0, sql.length());
		//		sql.append("select b.name,b.pk_dept,b.def1,case when b.pk_fatherorg='~' then null else b.pk_fatherorg end pk_fatherorg,'49238782' parentid from org_dept b left join org_corp a on a.pk_corp = b.pk_org where a.pk_corp =(select pk_corp from org_corp where name = '广证领秀投资有限公司') and nvl(b.dr,0)=0 and b.hrcanceled = 'N'");
